...
**The Codex Desktop app spawns each new session inside an isolated git worktree under `.Codex/worktrees/<random-name>/`. Files written there are invisible to other sessions and to the user's main repo. This silently breaks every multi-session plan if not handled.**
...
If the output contains `.Codex/worktrees/`, you are in an isolated worktree. **Immediately switch to the main repo:**

```bash
cd "/Users/niles/Library/Mobile Documents/com~apple~CloudDocs/Projects/ForcingFunction"
pwd   # confirm path no longer contains ".Codex/worktrees/"
```

**All file reads, edits, commits, and planning-doc updates from this point on MUST happen at the main repo path above.** Never write inside `.Codex/worktrees/*`. If a later tool call lands you back in a worktree path (cwd resets), `cd` back to the main repo path first.
